...
const PetCommunity = ({ username, onLogout }) => {
  ...
  const [comments, setComments] = useState([
    // Example comments data
    {
      user: 'John Doe',
      date: '2023-01-01',
      comment: 'Had a great experience with the Veterinary Clinic! The staff is friendly and caring.',
    },
    {
      user: 'Jane Smith',
      date: '2023-02-15',
      comment: 'Took my pet for a checkup and received excellent service. Highly recommended!',
    },
  ...
  const addComment = () => {
    if (newComment.trim() !== '') {
      ...
      const newComments = [...comments, { user: username, date: currentDate, comment: newComment }];
  ...
  return (
    <div style={{ display: 'flex', backgroundColor: '#f8f9f9', minHeight: '100vh', padding: '20px' }}>
      ...
      <div style={{ marginLeft: '20%', padding: '20px', width: '80%' }}>
        <h5 style={{ fontWeight: 'lighter' }}>Dashboard / Pet Community</h5>

        {/* Comment Form */}
        <div style={{ marginBottom: '20px', backgroundColor: '#fff', borderRadius: '8px', padding: '15px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
          <h4 style={{ color: '#e98821' }}>Add Your Comment</h4>
          <textarea
            rows="3"
          ...
          ></textarea>
          <button onClick={addComment} style={{ backgroundColor: '#e98821', color: '#fff', padding: '8px 12px', borderRadius: '5px', border: 'none', cursor: 'pointer' }}>
            Add Comment
          </button>
        </div>
        ...
        {comments.map((comment, index) => (
          <div key={index} style={{ marginBottom: '20px', backgroundColor: '#fff', borderRadius: '8px', padding: '15px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
              {/* You can replace the profile picture with an actual image */}
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#e98821', marginRight: '10px' }}></div>
              <h4 style={{ color: '#e98821' }}>{comment.user}</h4>
            </div>
            <p style={{ marginBottom: '5px', color: '#555' }}>{comment.date}</p>
            <p>{comment.comment}</p>
          </div>
        ))}
      </div>
